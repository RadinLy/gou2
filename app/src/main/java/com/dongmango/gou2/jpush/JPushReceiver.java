...
public class JPushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            Log.d(TAG, "[MyReceiver] 接收到推送下来的通知的内容: " + data);
        } else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(intent.getAction())) {
            Log.d(TAG, "[MyReceiver] 用户点击打开了通知");
            //打开自定义的Activity
            if (MainTabActivity.mMainTabActivity == null) {
                String data = bundle.getString(JPushInterface.EXTRA_EXTRA);
                PreferenceUtil.setPrefString(context, "JPUSH_JSON", data);

                Log.d(TAG, "[MyReceiver] 跳转主页面");
...
//                i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                ...
                Log.d(TAG, "[MyReceiver] 执行主页面方法");
                String data = bundle.getString(JPushInterface.EXTRA_EXTRA);
                MainTabActivity.mMainTabActivity.toJPushAct(data);
            }
